...
def buy_or_sell(session, expected_return):
    for i in expected_return.keys():
        if expected_return[i] > .005:
            session.post('http://localhost:9999/v1/orders', params = {'ticker': i, 'type': 'MARKET', 'quantity': 100, 'action': 'BUY'})
        session.post('http://localhost:9999/v1/orders', params = {'ticker': i, 'type': 'MARKET', 'quantity': 100, 'action': 'SELL'})
